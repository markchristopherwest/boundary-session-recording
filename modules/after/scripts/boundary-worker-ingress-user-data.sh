...
worker {
  ...
  tags {
    type = ["$id_instance", "upstream", "ingress"]
  }
...
echo "Generating upstream.hcl for Worker..."
echo "$stanza_product" > "/home/boundary/upstream-worker.hcl"

config_service=$(cat <<EOF
[Unit]
Description=Boundary Worker (Ingress Upstream)

[Service]
ExecStart=/bin/boundary server -config /home/boundary/upstream-worker.hcl
User=boundary
Group=boundary
